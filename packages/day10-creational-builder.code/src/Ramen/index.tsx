...
export const IChiRanRamen: React.FC<I_Props_IChiRanRamen> = ({ onSubmit }) => {
    ...
    return (
        <form onSubmit={handleSubmitOrder} id="ramenOrder">
            <div>
            ...
            <div className={allDefault ? 'disable' : 'enable'}>
                ...
                <select value={options.taste} id="taste" name="taste" onChange={handleOptionsChange}>
                ...
                <div>
                    ...
                    <select value={options.greenOnion} id="greenOnion" name="greenOnion" onChange={handleOptionsChange}>
                    ...
                    <input
                        type="checkbox"
                        name="doubleGreenOnion"
                        checked={doubleGreenOnion}
                        onChange={e => setDoubleGreenOnion(!doubleGreenOnion)}
                    ></input>
                </div>
                <div>
                    <label htmlFor="slicedPork">叉燒：</label>
                    <select value={options.slicedPork} id="slicedPork" name="slicedPork" onChange={handleOptionsChange}>
                        <option value={1}>有(建議)</option>
                        <option value={0}>無</option>
                    </select>
                    ...
                    <input
                        type="checkbox"
                        name="doubleSlicedPork"
                        checked={doubleSlicedPork}
                        onChange={e => setDoubleSlicedPork(!doubleSlicedPork)}
                    ></input>
                </div>
                <label htmlFor="spicy">赤紅秘製醬汁：</label>
                <input type="number" value={options.spicy} min="0" max="2" step="0.5" name="spicy" onChange={handleOptionsChange}></input>
                <label htmlFor="noodle">麵的硬度：</label>
                <select value={options.noodle} id="noodle" name="noodle" onChange={handleOptionsChange}>
